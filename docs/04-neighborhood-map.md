...
| 物理フリート | Open-RMF | 複数フリートと設備の相互運用、上位調停 | 個体帰属の機体状態と外部診断参照を扱う将来プロファイルが必要 |
...
4. **原則を変更不能な根底原則と、Narrativeの時制から現れる可変原則へ分ける。**
5. **一つの正典Narrative系列を、系の専用処理が固定Revisionとしてコミットする。**
6. **Narrativeの解釈・判断を系へ帰属させ、計算実行者の来歴と分ける。**
7. **Handoffを例外処理ではなく一級操作にする。**
8. **出発個体が消えても、系が外部化資料からHandoffを再構成する。**
9. **宛先未確定Handoffを、受領Eventによって後継個体へ束縛する。**
10. **完了申告と反証が食い違う場合、未確定状態を潰さない。**
11. **他個体のMemoryEntryを、Recallから出力まで自己経験へ変えない。**
12. **ライフサイクルを信頼済みローカルユーザー起点に限定しながら、Narrativeと可変原則の主権を系に残す。**
...
- 可変原則の改訂アルゴリズムを実装済みであること
- 忘却・隔離・想起濃度の自動制御を実装済みであること
- モデル固有傾向への一般化規則を持つこと
- 外部直接改変を完全に検出できること
- Narrative喪失から同じ系を復旧できること
